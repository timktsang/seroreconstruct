...
test_that("table_infections errors for multi", {
  expect_error(table_infections(test_fit_multi), "not supported")
})

test_that(".get_param_names returns correct count", {
  names <- seroreconstruct:::.get_param_names(test_fit)
  expect_equal(length(names), ncol(test_fit$posterior_model_parameter))
})
